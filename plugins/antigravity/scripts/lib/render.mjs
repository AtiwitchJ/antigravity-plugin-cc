...
export function renderTaskResult({ text, failureMessage, reasoningSummary }, { title, jobId = null, write = false, agentName = "Antigravity" } = {}) {
  const lines = [
    `# ${title ?? "Kilo task"}`,
...
export function renderReviewResult(text, { reviewLabel = "Review", targetLabel = "working tree", sessionId = null, agentName = "Antigravity" } = {}) {
  const lines = [
    `# ${reviewLabel}`,
    `- Target: ${targetLabel}`,
...
export function renderStoredJobResult(job, storedJob, { agentName = "Antigravity" } = {}) {
...
export function renderTransferResult({ threadId, resumeCommand, agentName = "Antigravity" }) {
  return [
    `# ${agentName} transfer`,
