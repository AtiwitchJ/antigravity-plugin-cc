...
const COMPANION = path.join(ROOT, "plugins", "antigravity", "scripts", "antigravity-companion.mjs");

function runCompanion(args) {
  return spawnSync(process.execPath, [COMPANION, ...args], { encoding: "utf8" });
}

test("antigravity-companion.mjs has no syntax errors (regression: was a broken, un-templated `$name-companion` literal)", () => {
  const result = runCompanion(["--help"]);
  assert.equal(result.status, 0);
  assert.doesNotMatch(result.stderr, /SyntaxError/);
});

test("antigravity-companion.mjs --help prints usage", () => {
  const result = runCompanion(["--help"]);
