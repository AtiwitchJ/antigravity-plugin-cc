...
test("marketplace.json exists and is valid JSON", () => {
  ...
  assert.equal(parsed.plugins[0].name, "antigravity");
});
...
test("marketplace.json entry has matching source dir and plugin.json", () => {
  ...
  assert.equal(json.name, "antigravity");
...
test("every command file exists", () => {
  ...
  const cmdDir = path.join(ROOT, "plugins", "antigravity", "commands");
...
test("rescue subagent exists", () => {
  const agentFile = path.join(ROOT, "plugins", "antigravity", "agents", "antigravity-rescue.md");
