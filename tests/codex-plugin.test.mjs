...
test("codex plugin manifest exists and is host-neutral", () => {
  ...
  assert.equal(parsed.name, "antigravity");
  assert.equal(parsed.version, "0.1.0");
  assert.equal(parsed.skills, "./skills/");
  assert.equal(parsed.author.name, "agents-plugin-cc");
  assert.equal(parsed.interface.displayName, "Antigravity");
...
test("codex marketplace entry points at the plugin source", () => {
  ...
  assert.equal(parsed.name, "agents-plugin-cc-antigravity");
  const entry = parsed.plugins.find((plugin) => plugin.name === "antigravity");
  assert.ok(entry, "Missing antigravity entry");
  assert.equal(entry.source.source, "local");
  assert.equal(entry.source.path, "./plugins/antigravity");
